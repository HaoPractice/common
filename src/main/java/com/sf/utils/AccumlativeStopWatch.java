 ...
 * and reducing the likelihood of calculation errors.
 *
 * <p>
 ...
 * @since May 2, 2001
 */
public class AccumlativeStopWatch {
  private static final String BLANK2 = "  ";
  private static final String BLANK3 = "   ";
  /**
   * Identifier of this stop watch. Handy when we have output from multiple stop watches and need to
  ...
  Map<String, TaskInfo> taskMap = new HashMap<>();
   ...
   * 
   * @param id identifier for this stop watch. Handy when we have output from multiple stop watches
   *        and need to distinguish between them.
   */
  public AccumlativeStopWatch(String id) {
    this.id = id;
  }
  ...
  /**
   * Determine whether the TaskInfo array is built over time. Set this to "false" when using a Test1
   * for millions of intervals, or the task info structure will consume excessive memory. Default is
   * "true".
   */
  ...
  public void start(String taskName) throws IllegalStateException {
    if (this.running) {
      throw new IllegalStateException("Can't start Test1: it's already running");
    }
    this.running = true;
  ...
  /**
   * Stop the current task. The results are undefined if timing methods are called without invoking
   * at least one pair {@code start()} / {@code stop()} methods.
   * 
   * @see #start()
   */
  public void stop() throws IllegalStateException {
    if (!this.running) {
      throw new IllegalStateException("Can't stop Test1: it's not running");
    }
    long lastTime = System.currentTimeMillis() - this.startTimeMillis;
    this.totalTimeMillis += lastTime;

    TaskInfo taskInfo;
    if (taskMap.containsKey(currentTaskName)) {
      taskInfo = taskMap.get(currentTaskName);
      taskInfo.timeMillis += lastTime;
    } else {
      taskMap.put(currentTaskName, taskInfo = new TaskInfo(this.currentTaskName, lastTime));
    }
    taskInfo.count++;
  ...
  public String shortSummary() {
    return "Test1 '" + getId() + "': running time (millis) = " + getTotalTimeMillis();
  }
  ...
  public String prettyPrint() {
    ...
    if (!this.keepTaskList) {
      ...
      for (TaskInfo task : taskMap.values()) {
        ...
        sb.append(nf.format(count)).append(str);
   ...
   * {@code getTaskInfo()} and use the task info directly.
   */
  @Override
